...
namespace RockTools
{
    ...
    public static class RockBaker
    {
        ...
        {
            var colliders = rockGenerator.pRoot.GetComponentsInChildren<Collider>(false)
                .Where(x => x.gameObject.activeSelf).ToArray();
            var meshFilters = rockGenerator.pRoot.GetComponentsInChildren<MeshFilter>(false)
                .Where(x => x.gameObject.activeSelf).Where(mf => mf.sharedMesh != null).ToArray();
        ...
        private static void ClearInsidePolys(MeshBuffer meshBuffer, Collider[] colliders, Collider skip)
        {
            ...
            for (var j = count - 1; j >= 0; j--)
            {
                ...
                if (CollisionCheck(colliders, skip, v1) && CollisionCheck(colliders, skip, v2) &&
                    CollisionCheck(colliders, skip, v3))
        ...
        private static void MergeCloseVertices(MeshBuffer meshBuffer, float threshold = 0.001f)
        {
            ...
            for (var i = 0; i < meshBuffer.triangleCount; i += 3)
            {
                for (var j = 0; j < 3; j++)
                {
                    ...
                    var foundIndex = vertices.FindIndex(x =>
                        Mathf.Abs((x - meshBuffer.vertices[vIndex]).sqrMagnitude) < sqrThreshold);
                    if (foundIndex == -1)
                    {
